...
func (ss *Sim) ConfigNet(net *leabra.Network) {
	...
	ctx := net.AddLayer2D("CTX", 12, 12, emer.Hidden)
	...
	ctx.SetRelPos(relpos.Rel{Rel: relpos.RightOf, Other: "DG", YAlign: relpos.Back, Space: 5})
	out.SetRelPos(relpos.Rel{Rel: relpos.FrontOf, Other: "CTX", YAlign: relpos.Front, Space: 5})
	//ext.SetRelPos(relpos.Rel{Rel: relpos.LeftOf, Other: "Input", YAlign: relpos.Front, Space: 2})
...
func (ss *Sim) SleepCyc(c [][]float64, stage string) {
	...
	for cyc := 0; cyc < 30000; cyc++ {
		...
		writecyc = append(writecyc, fmt.Sprint(ss.TrainEnv.Run.Cur), fmt.Sprint(ss.TrainEnv.Epoch.Cur), fmt.Sprint(ss.SleepCounter), fmt.Sprint(ss.PlusPhase), fmt.Sprint(ss.MinusPhase), fmt.Sprint(minABA), fmt.Sprint(ABAMatch), fmt.Sprint(minABB), fmt.Sprint(ABBMatch),
...
func (ss *Sim) SatMatch(inpact, outact []float32) (int, int, float32, float32, int, int, float32, float32) { // this has bugs! need to fix
	...
	file, err = os.Open("ACPats_nohead.tsv")
	if err != nil {
		fmt.Println("err in reading file")
	}
	...
	// Thresholded version
	//inpbin := []int{}
	//for _, val := range inpact {
	//	if val > 0.5 {
	//		inpbin = append(inpbin, 1)
	//	} else {
	//		inpbin = append(inpbin, 0)
	//	}
	//}
	...
	//outbin := []int{}
	//for _, val := range outact {
	//	if val > 0.5 {
	//		outbin = append(outbin, 1)
	//	} else {
	//		outbin = append(outbin, 0)
	//	}
	//}
